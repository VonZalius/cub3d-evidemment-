...
void	initial_step(t_progr *cube, t_element *node)
{
	...
	{
		node->step_x = -1;
		node->side_x = ((cube->player.co_x / cube->map.tiles_edge) - node->map_x) * node->delta_x;
	}
	else
	{
		node->step_x = 1;
		node->side_x = (node->map_x + 1.0 - (cube->player.co_x / cube->map.tiles_edge)) * node->delta_x;
	}
	if (node->ray_dir_y < 0)
	{
		node->step_y = -1;
		node->side_y = ((cube->player.co_y / cube->map.tiles_edge) - node->map_y) * node->delta_y;
	}
	else
	{
		node->step_y = 1;
		node->side_y = (node->map_y + 1.0 - (cube->player.co_y / cube->map.tiles_edge)) * node->delta_y;
	}
}
...
void	calc_final_ray(t_progr *cube, t_element *node)
{
	if (node->side == 0)
	{
		node->perp_wall_dist = (node->map_x - (cube->player.co_x / cube->map.tiles_edge)
				+ (1 - node->step_x) / 2) / node->ray_dir_x;
		node->wall_x = (cube->player.co_y / cube->map.tiles_edge) + node->perp_wall_dist * node->ray_dir_y;
	}
	else
	{
		node->perp_wall_dist = (node->map_y - (cube->player.co_y / cube->map.tiles_edge)
				+ (1 - node->step_y) / 2) / node->ray_dir_y;
		node->wall_x = (cube->player.co_x / cube->map.tiles_edge) + node->perp_wall_dist * node->ray_dir_x;
	}
